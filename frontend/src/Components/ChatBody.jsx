...
import { ALLOW_FILE_UPLOAD, HTTP_CHAT_API, FEEDBACK_API } from "../utilities/constants";
...
function ChatBody() {
  ...
  const askBot = async (question) => {
    ...
    try {
      ...
      console.log("🔵 Sent HTTP payload:", payload);
      console.log(`⏱️  Start time: ${new Date().toISOString()}`);

      const response = await fetch(HTTP_CHAT_API, {
        method: 'POST',
        headers: {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (data.type === 'thinking') {
              // Immediate acknowledgment - message received and processing started
              if (thinkingTime === null) {
                thinkingTime = Date.now() - startTime;
                console.log(`💭 Thinking indicator received in ${thinkingTime}ms: ${data.status}`);
              }
              // Keep status as PROCESSING to show loading indicator
              ...
              console.log(`📦 Chunk #${chunkCount} (${data.chunk.length} chars): "${data.chunk}"`);
              ...
              // Citations metadata (optional, can be ignored if citations come in complete)
              console.log(`📚 Received ${data.citations?.length || 0} citations`);
              ...
              const streamingTime = firstChunkTime ? totalTime - firstChunkTime : 0;

              // Log performance metrics
              console.log('\n' + '═'.repeat(60));
              console.log('📊 STREAMING PERFORMANCE METRICS');
              console.log('═'.repeat(60));
              console.log(`💭 Time to Thinking: ${thinkingTime || 'N/A'}ms`);
              console.log(`⚡ Time to First Chunk: ${firstChunkTime || 0}ms`);
              console.log(`⏱️  Total Response Time: ${totalTime}ms`);
              console.log(`📦 Total Chunks: ${chunkCount}`);
              console.log(`📝 Total Characters: ${totalChars}`);
              if (chunkCount > 0) {
                console.log(`📊 Avg Chunk Size: ${(totalChars / chunkCount).toFixed(1)} chars`);
                console.log(`⚡ Streaming Time: ${streamingTime}ms`);
                console.log(`📈 Streaming Speed: ${(totalChars / (streamingTime / 1000)).toFixed(0)} chars/sec`);
              }
              ...
              replaceProcessing(`Error: ${data.message}`);
              setProcessing(false);
            }
          } catch (err) {
      ...
      replaceProcessing("Connection error. Please try again.");
      setProcessing(false);
    }
  };
