...
export class LearningNavigatorStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const knowledgeBaseDataBucket = s3.Bucket.fromBucketName(this, 'KnowledgeBaseData', 'mhfa-chatbot-docs-1769547754');
    ...
    const chatResponseHandler = new lambda.Function(this, 'chatResponseHandler', {
      ...
      handler: 'handler-http-streaming.handler', // HTTP streaming handler with Lambda response streaming
      code: lambda.Code.fromAsset('lambda/chatResponseHandler'),
      architecture: lambdaArchitecture,
      memorySize: 1536, // Increased memory for faster execution
      environment: {
    ...
    // Add Lambda Function URL for HTTP streaming (required for awslambda.streamifyResponse)
    const chatFunctionUrl = chatResponseHandler.addFunctionUrl({
    ...
    sessionLogsTable.grantReadWriteData(chatResponseHandler); // NEW: Grant DynamoDB access for streaming handler
    ...
    kbSyncLambda.addPermission('AllowS3Invoke', {
      ...
      action: 'lambda:InvokeFunction',
      sourceAccount: this.account,
      sourceArn: `arn:aws:s3:::mhfa-chatbot-docs-1769547754`,
    });
    ...
    // Add HTTP streaming chat endpoint (no auth for public chatbot)
    const chatResource = AdminApi.root.addResource('chat');
    const chatIntegration = new apigateway.LambdaIntegration(chatResponseHandler, {
      proxy: true,
    ...
    // *** AMPLIFY DEPLOYMENT DISABLED - Frontend will be deployed manually ***
    ...
    new cdk.CfnOutput(this, 'HttpChatEndpoint', {
      value: `${AdminApi.url}chat`,
      description: 'HTTP Streaming Chat endpoint via API Gateway (non-streaming)',
    });

    new cdk.CfnOutput(this, 'ChatFunctionUrl', {
      value: chatFunctionUrl.url,
      description: 'Lambda Function URL for HTTP Response Streaming (use this for streaming)',
    });
  }
}
